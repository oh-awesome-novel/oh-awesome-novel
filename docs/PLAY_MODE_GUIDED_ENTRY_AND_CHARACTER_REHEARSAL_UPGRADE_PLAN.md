...
>
> 当前阶段排除：MuseAI P2 的沉浸与跨设备增强，包括 LAN / 移动端 / 跨设备、头像与语音、视觉附件、沉浸式演出、面向用户暴露的分阶段模型配置，以及高级 setup 升级 / 重建体验；产品尚未发布，旧 endpoint / 数据 migration 和系统化无障碍支持同样暂缓。
>
> 分析日期：2026-07-14。
>
> 执行顺序调整：2026-07-15。
>
> 当前执行原则：基础设施已经足够。世界事件 M1 与本计划 F1 已于 2026-07-15 完成，世界事件 M2–M4、本计划 F2 / F3 已于 2026-07-16 完成；世界事件 M5 长篇使用与体验收口和本计划 F4 advanced Director controls 已于 2026-07-20 完成。接下来优先进入真实使用反馈；耐久性、发布迁移和通用 transport 加固只在具体风险触发时成为局部门槛。
...
2. **按功能触发的局部 correctness gate**：F1 participant perception 必须带不可变、scene-local 的初始知识 evidence snapshot；F4 持久化 `grantKnowledge` 必须复用 M3 已落地的 branch-local knowledge / reveal store，并扩展 participant-scoped typed transition。attempt finalize 必须复用现有 settlement validator、hard-due evaluator、session revision recheck 与一次提交；source-based setup 必须校验 ref、hash、missing / stale。旧 session migration 不在当前未发布产品的功能切片内，发布前若确需兼容再建立独立 task。
3. **非阻塞并行加固**：F1 已按局部 correctness gate 增加 cooperative cross-process session / attempt filesystem lock、持锁 CAS、staged attempt publish 与 crash self-healing；M5 已完成 summary / windowing 与 renderer 功能旅程。fsync、完整跨进程故障矩阵、可重启 terminal registry、graceful shutdown、通用 deadline / backpressure 和旧 endpoint deprecation 继续留在发布 hardening。
...
目录所有权必须明确：committed checkpoint / variant 已由 `turns/*.yaml`、parent link、branch snapshot 与 `selectedTurnIds` 表达，本计划不得再创建 `checkpoints/` 或 `variants/` 平行事实目录。F1 使用 versioned `scene-rehearsal.yaml`、committed `scenes/` evidence 与 `.recovery/turn-attempts/`；F2 增加 `play-setups/`；F3 增加 `reports/` 与 workspace-level `writing-references/`；F4 已增加可重建的 `memories/`。attempt-local step graph、recovery shadow 和 memory projection 都不能变成另一套 committed checkpoint / variant 实现。
...
当前产品尚未发布，F1–F4 不执行这类旧 session 转换；本节只保留发布前若需要兼容时的设计约束，不代表当前待办。
...
- 四个控制、actor queue 与状态变化保留已存在的原生控件 / 状态语义基线；不在当前阶段扩展系统化无障碍矩阵。
...
状态：**Completed（2026-07-20）**。实现与验收记录见 `docs/tasks/1190.md` 和 `docs/superpowers/plans/2026-07-20-play-advanced-director-f4.md`。
...
- visibility-aware Scene Memory、stale / rebuild、高级控制功能旅程与长列表体验；系统化键盘 / screen-reader 验收留待后续产品阶段。
...
实现记录：四类 append-only intervention、effect fingerprint、earlier-step suffix invalidation、dynamic / hybrid scheduler、typed `noMaterialEffect` / stagnation、复用既有 due evaluator 的 provisional overlay、M3 participant grant、visibility-aware Scene Memory、宿主重裁决 redirect、before-step perception constraint closure、strict Client 与 Desktop 七控制均已落地。合并验证为 95 个测试文件 / 680 项测试，Desktop production build、真实 `PlayWorkspace.vue` Electron renderer smoke、support TypeScript 检查与 `git diff --check` 通过。
...
- provider / source deadline、SSE backpressure 与更广的 bounded-read hardening；summary / detail 与 Play 主路径 windowing 已由 M5 完成。
- migration confirmation 与旧 endpoint deprecation 统一留待发布前；当前未发布产品不做兼容转换。
- 更广的真实应用 browser journey 与跨进程故障矩阵。
...
- stable participant refs；guest / legacy migration 暂缓到发布前。
...
- F1 四个控制与 actor queue 的既有基础控件 / 状态语义回归；不扩展 screen-reader / reduced-motion 矩阵。
- F4 七控制、typed forms、Lens、suffix / stagnation 与 Scene Memory 的功能旅程；系统化 keyboard-only / screen-reader 矩阵暂缓。
