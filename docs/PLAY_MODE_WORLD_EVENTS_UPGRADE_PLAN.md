...
> 里程碑状态：M1–M5 Completed
>
> 文档状态：Needs Review（并行 correctness lane 仍持续复核）
>
...
> 执行顺序调整：2026-07-15。
>
> 当前执行原则：现有 session / artifact / staged snapshot、专用 SSE 与顶级 Play 工作区已经满足继续交付产品功能的门槛。M1 与跨计划 F1 已于 2026-07-15 落地；M2–M4、跨计划 F2 / F3 已于 2026-07-16 落地；M5 长篇使用与体验收口及跨计划 F4 advanced Director controls 已于 2026-07-20 落地。产品当前未发布，旧 endpoint / 数据 migration 与系统化无障碍支持暂缓；后续只在真实使用触发具体风险时推进局部 correctness / release hardening。

## 当前产品阶段说明

OAN 目前仍处于产品早期开发阶段，尚未发布，也没有需要兼容的已发布用户数据。现阶段优先验证 Play Mode 的核心玩法、交互闭环与真实使用价值，以下工作明确暂缓，不作为当前功能里程碑、验收或合并的前置条件：

- **系统化无障碍支持**：暂不建设完整的 screen-reader、keyboard-only、焦点管理、公告语义、对比度矩阵及跨平台无障碍验收。新功能仍应保留合理的原生控件语义和基本可操作性，不主动制造明显退化。
- **旧 endpoint 整理**：暂不要求下线、重定向、兼容或标记废弃旧 endpoint；当前产品主路径只需使用新的 bounded / typed API。旧入口不应继续承载新功能，也不为尚未发布的调用方增加兼容层。
- **旧格式与数据 migration**：暂不实现旧 session / artifact 的批量转换、migration UI、确认流程、兼容 fixture 或发布数据修复。开发期数据允许随 schema 演进后重建，但当前格式仍须 fail closed，不能静默误读或损坏文件。

这些范围应在进入外部测试或发布准备阶段，或真实用户数据、兼容调用方、具体无障碍需求出现时重新建立独立 task 评估；在此之前，不以“顺手完善”为由扩张 M1–M5 或 F1–F4 的实现范围。
...
本升级不把世界事件范围悄悄并入原 `1090` 历史。`1090` 保留早期 UI / adoption 复核记录，`1120` 记录底座、branch UX 与并行 correctness 范围；M3 knowledge / reveal、M4 adoption 与 M5 长篇收口分别由 `1160`、`1170`、`1180` 记录。
...
- workspace mode 与布局偏好按 workspace 恢复，session rail 具备基础原生控件语义。根目录 `__test__/desktop-ui` 已覆盖 stream reducer、terminal 幂等、cancel / commit race、中性设计、router 恢复、mounted session rail、长 session 功能旅程与 Electron renderer smoke；系统化键盘 / screen-reader 矩阵按当前产品阶段暂缓。
...
4. 已完成 M3 branch-local knowledge / reveal、F3 evidence-backed outcome / writing handoff 与 M4 unified adoption / Human Approval 路径。
5. 已完成 M5：summary / selected detail、windowing、turn-scoped context trace、source drift choices 与 renderer 功能旅程已落地；旧 endpoint 下线、migration 与完整无障碍矩阵按当前产品阶段暂缓。
...
1. hard-due 与 M1 eligible event 已能推进世界，M2 已补齐原因 / 影响卡片，M5 已补齐 turn-scoped actual source trace 与长列表诊断。
2. typed 相对等待已有一等入口；绝对 `atWorldTime` 仍需和规范化 comparator 一起交付，宿主继续拒绝猜测自然语言时间顺序。
3. M3 已交付 branch-local knowledge / reveal store 与正式的 `playerUnknown -> rumor / playerVisible` 因果揭示链；F4 已增加 participant-scoped `grantKnowledge`。
4. M2 已把 checkpoint / restore / Retry 收口为命名 checkpoint、可见初始世界与统一 worldline；M5 已增加长 session renderer 功能旅程回归。
5. F3 / M4 已把 event、observation 与 outcome 统一带入 evidence-backed preview 和既有 Human Approval；新 canonical target 仍明确排除。
6. context trace、omitted source、canonical drift 与长 session windowing 已形成面向用户的诊断；当前剩余项主要是发布前迁移、旧入口下线及被真实使用触发的局部 hardening。
...
当前已落地 committed-turn 隐式 checkpoint：任意带完整 branch snapshot 的 artifact 都可作为 selected path 恢复目标，legacy 仅允许恢复到 branch base head。restore 后 session revision 单调推进，目标之后的 ledger 保留并标记为 variant；再次选择旧 variant 或从恢复点继续推进都不需要复制或删除历史。完整 `worldSettlement` 还可直接发起原子 provider Retry：宿主从 source artifact 读取 typed action，在同一 before-turn projection 上生成 sibling，旧结果不覆盖；首回合以共享虚拟 branch base 的完整 v2 roots 表达。M2 已补齐命名 / 可见初始 checkpoint，M3 knowledge / reveal 也随 branch snapshot、Restore 与 Retry 恢复，M5 进一步补齐 turn-scoped context trace 与 source drift 决策，因此本节冻结范围已经完成。
...
状态：**Completed（2026-07-16）**。实现与验收记录见 `docs/tasks/1160.md` 和 `docs/superpowers/plans/2026-07-16-play-branch-local-knowledge-m3.md`。
...
状态：**Completed（2026-07-16）**。实现与验收记录见 `docs/tasks/1170.md` 和 `docs/superpowers/plans/2026-07-16-play-evidence-adoption-m4.md`。
...
状态：**Completed（2026-07-20）**。实现与验收记录见 `docs/tasks/1180.md` 和 `docs/superpowers/plans/2026-07-20-play-long-session-m5.md`。

范围：

- session summary + selected detail、transcript / event feed windowing。
- Electron renderer 级 Play 功能 journey；系统化键盘 / screen-reader 验收留待后续产品阶段。
- turn-scoped context trace 记录实际使用、遗漏和发生漂移的 source。
- canonical source drift 提供继续当前内容、重新装配或 fork 的明确选择。
- context / source 状态的简洁诊断；新 UI 只走 bounded 主路径，旧 endpoint / migration 入口下线留待发布前。
...
- legacy migration confirmation 与旧 endpoint 下线（产品发布前再统一处理）。
...
- 当前仅保留基础原生控件语义与功能 smoke；系统化键盘、screen-reader、对比度和 reduced-motion 验收暂缓。
...
- [x] branch-local knowledge、显式 reveal link 与随分支恢复完成。
...
- [x] knowledge / reveal 的分支语义与 turn-scoped actual source trace 完成。
- [x] canonical adoption 继续经过 PendingAction / diff / human approval。
- [x] event / observation / outcome 使用同一 identity-only evidence closure 与显式 Review 流程；Director 查看真实 canonical diff，Player 先查看不含 canonical baseline 的 proposal diff，真实 PendingAction diff 只在明确进入审批界面后展示；accept 前 canonical 文件不变。
...
4. 已完成：branch-local knowledge 与 reveal link，让隐藏变化可以安全地先发生、后揭示。
5. 已完成 F3 evidence-backed Outcome / Writing Reference 与 M4 unified adoption preview / 人工审批路径。
6. 已完成 M5 长 session windowing、turn-scoped context trace、source drift 选择与 renderer Play journey；跨计划 F4 advanced Director controls 也已按独立 task 完成。

这些能力按纵向切片交付后，Play Mode 已从“具备正确底座”成长为“世界会回应、作者能控制、结果可用于写作，并能承受长 session”的顶级功能。当前更适合进入真实使用反馈；耐久性、发布迁移与系统化无障碍加固保持后续候选，但不再默认抢占产品功能主线。
