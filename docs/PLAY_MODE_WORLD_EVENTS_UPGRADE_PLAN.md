...
> 执行顺序调整：2026-07-15。
>
> 当前执行原则：现有 session / artifact / staged snapshot、专用 SSE 与顶级 Play 工作区已经满足继续交付产品功能的门槛。M1 与跨计划 F1 已于 2026-07-15 落地；M2 worldline / event explanation、M3 branch-local knowledge / reveal、M4 evidence-backed adoption、跨计划 F2 Source-backed Guided Start 与 F3 Outcome / Writing Handoff 已于 2026-07-16 落地。下一功能候选进入 M5 长篇使用与体验收口；事务耐久性与迁移 UI 仍作为并行 correctness lane。
...
本升级不把世界事件范围悄悄并入原 `1090` 历史。`1090` 保留早期 UI / adoption 复核记录，`1120` 记录底座、branch UX 与并行 correctness 范围；已完成的 M3 knowledge / reveal 与 M4 adoption 分别由 `1160`、`1170` 记录，M5 开始前应建立独立 task。
...
5. 下一候选 M5 按真实长篇使用压力补 context trace、source drift choices、summary / windowing 与完整旅程自动化；开始前建立独立 task。
...
1. hard-due 与 M1 eligible event 已能推进世界，M2 已补齐原因 / 影响卡片；turn-scoped actual source trace 与长列表诊断留在 M5 收口。
2. typed 相对等待已有一等入口；绝对 `atWorldTime` 仍需和规范化 comparator 一起交付，宿主继续拒绝猜测自然语言时间顺序。
3. M3 已交付 branch-local knowledge / reveal store 与正式的 `playerUnknown -> rumor / playerVisible` 因果揭示链；participant-scoped `grantKnowledge` 留在 F4。
4. M2 已把 checkpoint / restore / Retry 收口为命名 checkpoint、可见初始世界与统一 worldline；浏览器级长旅程回归留在 M5。
...
- [x] knowledge / reveal 的分支语义完成；turn-scoped actual source trace 留在 M5。
...
6. 下一候选 M5 收口长 session windowing、turn-scoped context trace、source drift 选择与浏览器级 Play journey；跨计划 F4 advanced Director controls 可按独立 task 推进。

这些能力按纵向切片交付后，Play Mode 已从“具备正确底座”成长为“世界会回应、作者能控制、结果可用于写作”的顶级功能；M5 继续负责长篇规模与真实旅程收口。耐久性和规模化加固保持并行推进，但不再默认抢占功能主线。
