...
>
> Superseding constraint: Play 与 Writing 是 workspace 内同层级的顶级模式。本文的 right review panel 只描述 Writing 工作台及共享审阅能力；不得再把完整 Play Mode 放入右栏 tab。
...
本布局位于 `Writing` 顶级模式之下。Workspace shell 还必须在更高一层提供：

```text
Workspace mode navigation
├── Writing -> 本文定义的 agent / file / review 工作台
└── Play    -> 独立 Play workspace
```
...
`Play` 明确不属于此表。Play 的 World HUD、Event、Context、Adoption 可以作为 Play workspace 自己的 inspector tabs，但完整 Play Mode 必须通过顶级导航进入。

## Top-level Writing And Play Modes

Workspace shell 的目标层级：
...
│   └── right review panel
└── PlayWorkspace
    ├── scene / cast navigation
    ├── transcript / action composer
    └── world HUD / event / context / adoption inspector
```

行为要求：

- `WorkspaceToolbar` 中的 Play 操作必须切换 `workspaceMode`，不能再发出 `openRightTab('play')`。
- `WorkspaceRightTab` 应移除 `play`；Writing review panel 不挂载 `PlayModeTab.vue`。
- Play session 的 streaming、checkpoint、branch 和未提交输入在模式切换时应安全保留。
- 当前模式和 Play session selection 应按 workspace 持久化；刷新后可恢复。
- Play 内部 inspector 可隐藏、调宽或在移动端变为 sheet，但关闭 inspector 不等于退出 Play。
- 顶级模式切换不得自动 adoption、提交 PendingAction 或修改 canonical 文件。
