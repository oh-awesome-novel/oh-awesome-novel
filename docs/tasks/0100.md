...
- Core workspace initialization creates `.oan/` and the canonical content directories in an empty workspace.
- Core exports stable numbered path helpers for both volume metadata and narrative chapter files.
- `chapter.get` and `summary.generateChapter` reject `0001/0000` because `0000.md` is reserved for volume metadata, not narrative chapter content.
- Sample workspace chapter and summary files use stable numbered paths.
