...
- Implemented as `workspace.writeFile` in `packages/tools`.
- Shadow writes are internally fixed under `workspace/.workspace/shadow-writes`.
- User-supplied paths still cannot target hidden files, `.oan`, or `.workspace`.
- This remains a temporary validation tool and is not the long-term write architecture.
